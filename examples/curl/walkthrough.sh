...
curl -s -X POST "$BASE/v1/effects/begin" "${AUTH[@]}" -d '{
  "effect_type":"email.send","idempotency_key":"welcome:u_1",
  "payload":{"to":"sam@example.test"}}' \
  | python3 -c 'import sys,json;d=json.load(sys.stdin);print("  decision:",d["decision"],"| retry after",d.get("retry_after_seconds"),"s")'

say "4. Do the real work, then report the outcome."
curl -s -X POST "$BASE/v1/effects/$EID/report" "${AUTH[@]}" \
  -d "{\"lease_token\":\"$TOK\",\"outcome\":\"succeeded\",\"result\":{\"message_id\":\"msg_77\"},\"actual_cost_micros\":780}" \
  | python3 -m json.tool
...
curl -s -X POST "$BASE/v1/effects/begin" "${AUTH[@]}" -d '{
  "effect_type":"email.send","idempotency_key":"welcome:u_1",
  "payload":{"to":"sam@example.test"}}' \
  | python3 -c 'import sys,json;d=json.load(sys.stdin);print("  decision:",d["decision"],"| result:",d.get("result"))'

say "6. Reusing the key with different arguments is refused."
curl -s -X POST "$BASE/v1/effects/begin" "${AUTH[@]}" -d '{
  "effect_type":"email.send","idempotency_key":"welcome:u_1",
  "payload":{"to":"someone-else@example.test"}}' \
  | python3 -c 'import sys,json;print("  error:",json.load(sys.stdin)["error"]["code"])'
...
curl -s -X POST "$BASE/v1/effects/begin" "${AUTH[@]}" -d '{
  "effect_type":"payment.charge","idempotency_key":"invoice:2026-08:acct_1",
  "payload":{"cents":4200}}' | python3 -m json.tool
...
curl -s -X POST "$BASE/v1/effects/$CID/resolve" "${AUTH[@]}" -d '{
  "outcome":"succeeded","evidence":"vendor dashboard shows one charge, ch_3Pab",
  "result":{"charge_id":"ch_3Pab"}}' | python3 -m json.tool
